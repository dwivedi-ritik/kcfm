...
STATE_FILE = os.path.expanduser("~/.kc.json")
CONF_FILE = os.path.expanduser("~/.kc.conf.json")
LOG_FILE = os.path.expanduser("~/.kc.log")

DEFAULTS = {
    "mark_time": 600,   # secs of inactivity before a session is marked
    "sweep_time": 300,  # secs after marking before it is frozen
    "interval": 120,    # secs between passes in `kc run`
}
...
def load_conf():
    """DEFAULTS overridden by ~/.kc.conf.json; unknown keys ignored."""
    data = _read_raw()
